%% figure2.m
%
% This script produces the six panels of figure 2.
%
%% Description
%
% The script computes the effects of a positive technology shock in two New Keynesian models:
%
...
dynare irfFairnessTechnology.mod
irfFairness = cell2mat(struct2cell(oo_.irfs));

%% IRFs for textbook New Keynesian model

dynare irfTextbookTechnology.mod
irfTextbook = cell2mat(struct2cell(oo_.irfs));

%% Annualize inflation rate

irfFairness(2,:) = 4.*irfFairness(2,:); 
irfTextbook(2,:) = 4.*irfTextbook(2,:);
...
horizon = [0:1:12];
variable = {'Technology level','Inflation rate','Perceived price markup','Actual price markup','Employment','Output'};
...
for iVariable = 1:nVariable
	...
	% Populate y-axis
	set(gca,'YLim',[-1,1.5],'YTick',[-1:0.5:1.5],'YTickLabel',['  -1%';'-0.5%';'   0%';' 0.5%';'   1%';' 1.5%']);	
